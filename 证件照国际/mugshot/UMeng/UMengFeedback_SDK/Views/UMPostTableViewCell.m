...
- (NSString *)humanableInfoFromDate: (NSDate *) theDate {
    ...
    if (delta < 60) {
        ...
        if (delta < 60) {
            // n分钟前
            info = [NSString stringWithFormat:@"%lu minutes ago", (unsigned long)delta];
        } else {
            delta = delta / 60;
            if (delta < 24) {
                // n小时前
                info = [NSString stringWithFormat:@"%lu hours ago", (unsigned long)delta];
            } else {
                delta = delta / 24;
