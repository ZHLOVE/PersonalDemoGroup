...
//    NSLog(@"topic and replies count: %d", self.topicAndReplies.count);
    ...
    self.infoLabel.text = [NSString stringWithFormat:@"QQ: %@ Phone: %@ \nEmail: %@ Other: %@",
...
- (UIView *)infoView {
    if (_infoView == nil) {
        ...
        infoLabel.text = [NSString stringWithFormat:@"QQ: %@ Phone: %@ \nEmail: %@ Other: %@",
                          [self.inputToolBar.contactInfo valueForKeyPath:@"contact.qq"],
                          [self.inputToolBar.contactInfo valueForKeyPath:@"contact.phone"],
                          [self.inputToolBar.contactInfo valueForKeyPath:@"contact.email"],
                          [self.inputToolBar.contactInfo valueForKeyPath:@"contact.plain"]];
        ...
        [infoButton setTitle:@"Update info" forState:UIControlStateNormal];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.topicAndReplies[indexPath.row][@"is_failed"] boolValue]) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Send again?"
                                                            message:nil
                                                           delegate:self
...
- (void)recordButtonDragExitAction:(UIButton *)sender {
    [self showCancelView];
    [sender setTitle:@"Hold to speak" forState:UIControlStateNormal];
}
    ...
    if ( UM_IOS_7_OR_LATER) {
        [[AVAudioSession sharedInstance] requestRecordPermission:^(BOOL granted) {
            if (granted) {
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Permission denied" message:@"Setting Permission for recording in Settings -> Privacy -> Microphone" delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
                [alert show];
            }
        }];
    } else {
...
- (void)startRecordWithSender:(UIButton *)sender {
    ...
    [sender setTitle:@"Release to stop" forState:UIControlStateNormal];
