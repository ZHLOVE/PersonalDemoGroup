...
- (instancetype)init {
    self = [self initWithQuality:AVCaptureSessionPresetHigh position:CameraPositionBack];
    if(self) {
    }
...
- (instancetype)initWithQuality:(NSString *)quality position:(CameraPosition)position {
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         ...
         if (imageSampleBuffer != NULL) {
             ...
             if(self.fixOrientationAfterCapture) {
                 image = [image fixOrientationWithSize:image.size];
             }
         }
