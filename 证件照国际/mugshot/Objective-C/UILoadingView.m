...
//查找顶层视图
- (UIView*)findTopView:(UIViewController *)controller {
    UIViewController* recentView = controller;
...
- (BOOL)Show:(UIViewController *)tar WithString:(NSString *)title inTop:(BOOL)isTop {
    ...
    if (!self.isVisible) {
        ...
        [self.loadingView showInView:(isTop ? [self findTopView:tar] : tar.view) animated:NO];
        rtn = true;
    }
    return rtn;
}
