 ...
 */
extern NSString * const TPIMNotification_SendMessageResult;

/**
 *  收取收到消息通知（用来接收所有收到的消息）
 */
extern NSString * const TPIMNotification_ReceiveMessage;
extern NSString * const TPIMNotification_MessageKey;

/**
 *  收取事件消息通知（用来接收所有收到的Event事件）
 */
extern NSString * const TPIMNotification_EventMessage;
extern NSString * const TPIMNotification_EventKey;

/**
 *  开始/结束播放语音通知
 */
extern NSString * const TPIMNotification_WillStartPlayVoiceMessage;
extern NSString * const TPIMNotification_DidFinishPlayingVoiceMessage;

#ifndef TPIMSendMessageObject
#define TPIMSendMessageObject @"message"
#endif
#ifndef TPIMSendMessageError
#define TPIMSendMessageError @"error"
#endif
 ...
 *  事件类型
 */
typedef NS_ENUM(NSUInteger, TPIMEventType) {
    kTPIMCreateGroupEvent = 8,        //创建群组
    kTPIMExitGroupEvent = 9,          //退出群组（自己主动退出）
    kTPIMAddGroupMemberEvent = 10,    //群组加入成员
    kTPIMDeleteGroupMemberEvent = 11, //删除群组成员
};

//-----------------------------------------------------------
// TPIMMessage 消息的抽象类，Message对象需要使用具体类来实例化
//-----------------------------------------------------------

@interface TPIMMessage : NSObject<NSCopying>
...
/*
 * @param fromId      消息的发送方userName(不需要填写)
 * @param fromName    消息的发送方展示名(不需要填写)
 */
@property(atomic, strong,readonly) NSString *fromId;
@property(atomic, strong) NSString *fromName;
@property(atomic, strong) UIImage *fromHeadImage;
@property(atomic, strong) NSString *fromHeadUrl;
@property(atomic, strong) NSString *fromphone;
 ...
 *  群聊时 roomid
 */
@property (atomic, strong, readonly) NSString *roomId;
 ...
 * 消息是群聊还是单聊(发送时必须填写)
 */
@property(atomic, assign) TPIMConversationType conversationType;

/*
 * 消息类型,消息的子类已经自动填写(不需要填写)
 */
@property(assign, readonly) TPIMMessageContentType contentType;
 ...
 *  是否为群聊消息
 */
@property(atomic, assign) BOOL isPublicMessage;
 ...
 *  发送消息接口
 *
 *  @param message       待发送的消息。使用时只能使用TPIMMessage的子类(TPIMContentMessage、TPIMVoiceMessage、TPIMImageMessage、TPIMCustomMessage、TPIMEventMessage)
 *                                   发送结果需要监听TPIMNotification_SendMessageResult通知
 */
 ...
 *  获取消息大图接口
 *
 *  @param TPIMImageMessage 图片消息
 *  @param progress         下载进度
 *  @param handler          结果回调。正常返回时resultObject对象类型为NSURL,内容为图片文件路径
 */
+ (void)downloadOriginImage:(TPIMImageMessage *)message
 ...
 *  @param handler          结果回调。正常返回时resultObject对象类型为NSURL,内容为图片文件路径
 */
+ (void)downloadThumbImage:(TPIMImageMessage *)message
              withProgress:(NSProgress *)progress
         completionHandler:(TPIMCompletionHandler)handler;
 ...
 */
+ (void)downloadVoice:(TPIMVoiceMessage *)message
         withProgress:(NSProgress *)progress
    completionHandler:(TPIMCompletionHandler)handler;
...
///-----------------------------------------------------------------
@interface TPIMMediaMessage : TPIMMessage <NSCopying>

/**
 *  上传进度对象,发送时,用于获取当前多媒体资源的上传进度
 */
@property(atomic, strong) TPIMMediaUploadProgressHandler progressCallback;
...
///-----------------------------------------------------------------
@interface TPIMContentMessage : TPIMMessage <NSCopying>
...
///-----------------------------------------------------------------
@interface TPIMEventMessage : TPIMContentMessage <NSCopying>

/**
 * event消息的类型,参照TPIMEventType Enum
 */
@property(atomic, assign) TPIMEventType type;
...
///-----------------------------------------------------------------
@interface TPIMCustomMessage : TPIMMessage <NSCopying>
...
///-----------------------------------------------------------------
@interface TPIMImageMessage : TPIMMediaMessage <NSCopying>
...
///-----------------------------------------------------------------
@interface TPIMVoiceMessage : TPIMMediaMessage <NSCopying>
 ...
 *  语音对应的文字
 */
@property(atomic, strong) NSString *voiceString;

@end


///-----------------------------------------------------------------
/// TPIMVideoMessage 视频消息,发送时必须填写:mediaData 、targetId、contentType、imgSize
///                  发送语音消息时需要传的对象
///                  接收时通过resourcePath获取语音消息,图片不存在时需要调用downloadVoice重新获取
///-----------------------------------------------------------------
@interface TPIMVideoMessage : TPIMMediaMessage <NSCopying>
