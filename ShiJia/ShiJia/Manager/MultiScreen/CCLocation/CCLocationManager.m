...
@property (nonatomic, strong) UIAlertView *alvertView;
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    ...
    DDLogInfo(@"%f--%f",newLocation.coordinate.latitude,newLocation.coordinate.longitude);
...
-(void)startLocation
{
    if([CLLocationManager locationServicesEnabled] && [CLLocationManager authorizationStatus] != kCLAuthorizationStatusDenied)
    {
        if (_manager == nil) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^(){
                ...
                // 降低能耗，更改定位精度为 kCLLocationAccuracyKilometer
    ...
    {
        if (!_alvertView) {
            _alvertView=[[UIAlertView alloc]initWithTitle:@"提示" message:@"需要开启定位服务,请到设置->隐私,打开定位服务" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
            [_alvertView show];
        }
        if (_locationBlock) {
