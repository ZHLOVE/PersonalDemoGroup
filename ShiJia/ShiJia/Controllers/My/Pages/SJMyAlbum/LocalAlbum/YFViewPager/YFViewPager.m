...
#ifdef DEBUG
#define DLog(s, ...) DDLogInfo( @"<%p %@:(%d)> %@", self, [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define DLog(s, ...)
#endif
...
//    _scrollView.backgroundColor = [UIColor whiteColor];
    ...
    if ([self.viewTag isEqualToString:@"MyListView"]) {
        _pageControl = [[UIView alloc]initWithFrame:CGRectMake(60, 0, rect.size.width-120, 40)];
...
//    _pageControl.backgroundColor = [UIColor whiteColor];
    ...
    UILabel *selectedLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, _pageControl.frame.size.height -3,-20+ _pageControl.frame.size.width/_pageNum, 3)];
    ...
    for (NSInteger i = 0; i < _views.count; i++) {
        ...
        _pageframe.size.width = _pageControl.size.width / _pageNum;
...
//        circleLabel.textColor = [UIColor whiteColor];
        ...
        if (button.selected) {
            [UIView animateWithDuration:0.3 animations:^{
                ...
                sframe.origin.x = button.frame.origin.x+10;
