 ...
 *  segment的高度
 */
const CGFloat   LPPageVCSegmentHeight           = 42.0f;
...
//        DDLogInfo(@"LPPageVCSegmentStyleDefault");
    }
...
//        DDLogInfo(@"LPPageVCSegmentStyleLineHighlight");
    }
}
...
#pragma mark - 创建editButton
    ...
    [editButton setBackgroundImage:[UIImage imageNamed:@"more"] forState:UIControlStateNormal];
    [editButton setBackgroundImage:[UIImage imageNamed:@"more"] forState:UIControlStateHighlighted];
...
- (void)setCurrentIndex:(NSInteger)currentIndex {
    ...
    if (_currentIndex != currentIndex) {
        ...
        if ([_delegate respondsToSelector:@selector(pageVC:didChangeToIndex:fromIndex:)]) {
            [_contentScrollView setContentOffset:CGPointMake(_currentIndex * CGRectGetWidth(_contentScrollView.frame), 0) animated:NO];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (index != _currentIndex) {
        [UIView animateWithDuration:0.3 animations:^{
            ...
            UILabel *currentLabel = (UILabel *)[_segmentContainerView viewWithTag:1000 + index];
            CGRect frame = currentLabel.frame;
