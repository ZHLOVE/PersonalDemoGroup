...
{
    //文件路径需要搞搞
    if (self.isPublicVoice) {
        NSString *documentDirectory = [TPFileOperation getPublicVoiceMessageFilePath];//音频缓存文件夹名字
        return [documentDirectory stringByAppendingPathComponent:[MLDataCache md5String:key]];
    }
    else{
        NSString *documentDirectory = [TPFileOperation getPrivateVoiceMessageFilePath];//音频缓存文件夹名字
        return [documentDirectory stringByAppendingPathComponent:[MLDataCache md5String:key]];
    }
