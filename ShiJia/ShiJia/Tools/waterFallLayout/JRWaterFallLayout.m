...
//static const CGFloat JRDefaultColumnMargin = 20; // 列间距
...
@interface JRWaterFallLayout()
{
    ...
    CGFloat JRDefaultRowMargin;  // 行间距
    CGFloat JRDefaultColumnMargin; // 列间距
...
-(void)setJRDefaultColumnCount:(CGFloat)JRDefaultColumnCount{
    _JRDefaultColumnCount = JRDefaultColumnCount;
    [self setupColumnHeightsArray];
    // 计算最大的Y值
    self.maxY = [self maxYWithColumnHeightsArray:self.columnHeights];
    if (_JRDefaultColumnCount == 2) {
        JRDefaultRowMargin = 30;
        JRDefaultColumnMargin = 20;
    }
    else{
        JRDefaultRowMargin = 10;
        JRDefaultColumnMargin = 10;
    }
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (row ==1 &&self.JRDefaultColumnCount==2) {
        y += height/2;
    }
    else if (row ==0 &&self.JRDefaultColumnCount==2) {
        y -= 20;
    }
...
{
    CGFloat height = 0;
    if(self.JRDefaultColumnCount==2){
        if (self.attrsArray.count%2 != 0 ) {
             height = 140;
        }
    }
    else if(self.JRDefaultColumnCount==3){
        if (self.attrsArray.count%3 != 0 ) {
            height = 180;
        }
    }
        
    return CGSizeMake(0, self.maxY +height+ [self edgeInsets].bottom);
}
...
{
    return _delegateFlags.didRespondColumnCount ? [self.delegate columnCountOfWaterFallLayout:self] : self.JRDefaultColumnCount;
}
