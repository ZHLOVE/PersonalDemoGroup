...
@optional
- (void)debug_animatedImage:(FLAnimatedGif *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(FLAnimatedGif *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(FLAnimatedGif *)animatedImage;
@end
#endif


@interface FLAnimatedGif ()
...
@property (nonatomic, strong, readonly) FLAnimatedGif *weakProxy;
...
- (instancetype)init
{
    FLAnimatedGif *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
...
{
    FLAnimatedGif *animatedImage = [[FLAnimatedGif alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    FLAnimatedGif * __weak weakSelf = self;
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[FLAnimatedGif class]]) {
        FLAnimatedGif *animatedImage = (FLAnimatedGif *)image;
...
@implementation FLAnimatedGif (Logging)
