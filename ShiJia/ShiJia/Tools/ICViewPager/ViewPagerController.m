...
//#define kTabsViewBackgroundColor [UIColor colorWithRed:0x43/255.0 green:0x50/255.0 blue:0x7B/255.0 alpha:1]
#define kTabsViewBackgroundColor kUIColor(0x2c, 0x3c, 0x5e, 1)
//#define kTabsViewBackgroundColor [UIColor redColor]
...
- (BOOL)isEqualToColor:(UIColor *)otherColor {
    ...
    UIColor *(^convertColorToRGBSpace)(UIColor *) = ^(UIColor *color) {
        if (CGColorSpaceGetModel(CGColorGetColorSpace(color.CGColor)) == kCGColorSpaceModelMonochrome) {
            ...
            CGColorRef color = CGColorCreate(colorSpaceRGB, components);
...
@property (nonatomic) BOOL useImageAsIndicator;
...
//    [bezierPath stroke];
    ...
    if (self.selected) {
        if (self.useImageAsIndicator && self.indicatorImageView == nil) {
            self.indicatorImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"点击效果"]];
            ...
            [self insertSubview:self.indicatorImageView atIndex:0];
            self.indicatorImageView.frame = self.bounds;
            self.indicatorImageView.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        }
    ...
    if (self.selected && self.triangleHeight > 0) {
        // add by jzb at 20151030
        bezierPath = [UIBezierPath bezierPath];
        
        // Draw the indicator
        [bezierPath moveToPoint:CGPointMake(CGRectGetWidth(rect)/2-self.triangleHeight, CGRectGetHeight(rect))];
        [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect)/2, CGRectGetHeight(rect) - self.triangleHeight)];
        [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect)/2+self.triangleHeight, CGRectGetHeight(rect))];
    ...
    if (self.selected && !self.useImageAsIndicator && self.triangleHeight == 0.0) {
...
@property (getter = isPageViewControllerChanging, assign) BOOL pageViewControllerChanging;
...
- (void)dealloc{
    ((UIScrollView *)[self.pageViewController.view.subviews objectAtIndex:0]).delegate = nil;
...
- (void)setActiveContentIndex:(NSUInteger)activeContentIndex {
    if (self.pageViewControllerChanging) {
        return;
    }
    self.pageViewControllerChanging = YES;
    ...
    NSMutableArray* oldControllers = [NSMutableArray arrayWithArray:self.pageViewController.viewControllers];
    [oldControllers removeObject:viewController];
    for (UIViewController* oldController in oldControllers) {
    ...
    if (activeContentIndex == self.activeContentIndex) {
        [self.pageViewController setViewControllers:@[viewController]
                                         ...
                                         completion:^(BOOL completed) {
                                             ...
                                             weakSelf.pageViewControllerChanging = NO;
                                            }];
...
- (NSNumber *)tabWidthWithDefault:(CGFloat)defaultWidth{
    CGFloat value = defaultWidth;
    if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
...
- (void)setNeedsReloadColors {
    ...
    CGFloat trangleHeight = [self.delegate viewPager:self valueForOption:ViewPagerOptionTriangleHeight withDefault:0.0];
    
    BOOL useImageForIndicator = [[NSNumber numberWithFloat:[self.delegate viewPager:self
                                          valueForOption:ViewPagerOptionImageIndicator withDefault:0]] boolValue];
...
//    if (![self.indicatorColor isEqualToColor:indicatorColor]) {
        ...
        [self.tabs enumerateObjectsUsingBlock:^(TabView *tabView, NSUInteger index, BOOL *stop) {
            ...
            tabView.useImageAsIndicator = useImageForIndicator;
            tabView.triangleHeight = trangleHeight;
...
- (void)defaultSettings {
    ...
    ((UIScrollView *)[self.pageViewController.view.subviews objectAtIndex:0]).bounces = NO;
...
//add by jhl 20170314
-(void)enableToScroll{
    ((UIScrollView *)[self.pageViewController.view.subviews objectAtIndex:0]).bounces = YES;
}
- (void)defaultSetup {
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        TabView *tabView = [[TabView alloc] initWithFrame:CGRectMake(0.0, 0.0, [[self tabWidthWithDefault:tabViewContent.bounds.size.width + 10] floatValue], [self.tabHeight floatValue])];
