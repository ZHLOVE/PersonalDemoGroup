...
- (void)initQRLine {
    ...
    qrLine  = [[UIImageView alloc] initWithFrame:CGRectMake(self.bounds.size.width / 2 - self.transparentArea.width / 2, self.bounds.size.height / 2 - self.transparentArea.height / 2-H_OFFSET, self.transparentArea.width, 1)];
    qrLine.image = [UIImage imageNamed:@"scanLine"];
   // qrLine.backgroundColor = [UIColor redColor];
...
- (void)show {
    ...
    [UIView animateWithDuration:kQrLineanimateDuration animations:^{
        ...
        CGFloat maxBorder = self.frame.size.height / 2 + self.transparentArea.height / 2 -H_OFFSET- 8;
        if (qrLineY > maxBorder) {
            
            qrLineY = self.frame.size.height / 2 - self.transparentArea.height /2-H_OFFSET;
        }
        qrLineY++;
    }];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect clearDrawRect = CGRectMake(screenDrawRect.size.width / 2 - self.transparentArea.width / 2,
                                      screenDrawRect.size.height / 2 - self.transparentArea.height / 2-H_OFFSET,
...
- (void)addCornerLineWithContext:(CGContextRef)ctx rect:(CGRect)rect{
    ...
    CGContextSetRGBStrokeColor(ctx, 255 /255.0, 255/255.0, 255/255.0, 1);//绿色
