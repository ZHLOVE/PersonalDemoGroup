...
- (void)setUpProperties
{
    ...
    preBtn.frame = CGRectMake(0, 0, 45, 30);
    [preBtn setTitle:@"使用" forState:UIControlStateNormal];
    [preBtn setTitleColor:kNavTitleColor forState:UIControlStateNormal];
    ...
    for (NSNumber *type in types) {
        ...
        [self.assetsLibrary enumerateGroupsWithTypes:[type unsignedIntegerValue]
                                          usingBlock:^(ALAssetsGroup *assetsGroup, BOOL *stop) {
                                              ...
                                              if (error != nil) {
                                                  if (alert == nil) {
                                                      alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"此应用程序没有权限来访问您的照片或视频，您可以在\"隐私设置\"里启用访问" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                                                      [alert show];
                                                  }
}
                                          }];
...
- (void)setShowsCancelButton:(BOOL)showsCancelButton{
    ...
    if (showsCancelButton) {
        ...
        [backButton setImage:[UIImage imageNamed:@"white_back_btn"] forState:UIControlStateNormal];
        [backButton addTarget:self action:@selector(cancelEventDidTouched) forControlEvents:UIControlEventTouchUpInside];
       ...
       /* UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithCustomView:backButton];
        ...
        [self.navigationItem setLeftBarButtonItems:[NSArray arrayWithObjects:negativeSpacer, [[UIBarButtonItem alloc] initWithCustomView:backButton]/*this will be the button which u actually need*/, nil] animated:NO];
    ...
    [[PhotoAlbumManager sharedManager] saveImage:image
                                 ...
                                 completionBlock:^(ALAsset *asset, NSError *error) {
                                     if (error == nil && asset) {
                                         ...
                                         if ([_delegate respondsToSelector:@selector(jkImagePickerController:didFinishPickingMediaWithInfo:)]) {

                                         [_delegate jkImagePickerController:self didFinishPickingMediaWithInfo:info];
                                         }
                                     }
...
- (UILabel *)titleLabel{
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = [HiTVConstants titleColor];
