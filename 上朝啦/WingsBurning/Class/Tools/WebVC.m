...
- (void)viewDidLoad{
    ...
    }else if ([self.titleString isEqualToString:@"福利"]){
        [self showFuLi];
    }
}
...
- (void)showServiceAgreement{
    ...
    NSURL* url = [NSURL URLWithString:@"https://www.shangchao.la/service.html"];
...
- (void)showPrivacyPolicy{
    CGRect tempFrame = self.view.frame;
    UIWebView* webView = [[UIWebView alloc]initWithFrame:CGRectMake(0, 0, tempFrame.size.width, tempFrame.size.height - 64)];
    NSURL* url = [NSURL URLWithString:@"https://www.shangchao.la/private.html"];
    NSURLRequest* request = [NSURLRequest requestWithURL:url];
    [webView loadRequest:request];
    [self.view addSubview:webView];
}
...
- (void)showFuLi{
    CGRect tempFrame = self.view.frame;
    UIWebView* webView = [[UIWebView alloc]initWithFrame:CGRectMake(0, 0, tempFrame.size.width, tempFrame.size.height - 64)];
    NSURL* url = [NSURL URLWithString:@"https://www.shangchao.la/money"];
    NSURLRequest* request = [NSURLRequest requestWithURL:url];
    [webView loadRequest:request];
    [self.view addSubview:webView];
}
