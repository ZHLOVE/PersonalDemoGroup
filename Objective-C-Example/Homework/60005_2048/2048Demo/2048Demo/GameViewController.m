...
- (void)GameViewInit{
    self.logoLabel.layer.cornerRadius = 10;
    self.logoLabel.layer.masksToBounds = YES;
    self.logoLabel.backgroundColor = k2048CellColor;
    self.view.backgroundColor = [UIColor whiteColor];
}
...
- (void)drawBackCell
{
    for(int i=0;i<N;i++) // 行
    {
        for (int j=0; j<N; j++) // 列
        {
            ...
            // 颜色
            cellView.backgroundColor = kRGBCellBackColor;
            // 圆角
    ...
    switch (g.direction) {
        ...
        case UISwipeGestureRecognizerDirectionRight: // 向右
        {
            for(int row= 0;row<N;row++)// 行
            {
                ...
                {
                    [self swipeCellRightByCellId:(row*N+col)];
                }
            }
        }
            break;
        ...
        {
            for(int row= N-2;row>=0;row--)// 行
            {
                for(int col = 0;col<N;col++)// 列
                {
                    [self swipeCellDownByCellId:(row*N+col)];
                }
            }
        }
            break;
        case UISwipeGestureRecognizerDirectionUp: // 向上
        {
            for(int row= 1;row<N;row++)// 行
            {
                for(int col = 0;col<N;col++)// 列
                {
                    [self swipeCellUpByByCellId:(row*N+col)];
                }
            }
        }
            break;
...
//    // 当前格子里数字
    ...
    BOOL bEmpty = YES;// 右边没有格子
    for(int i = y+1;i<N;i++) // 从这个格子的右边一个格子开始往前面判断
    {
        if([self.boxArr[i+x*N] intValue]> 0)// 有数字
        {
            // 数字相等 且 前面那个数字不是合并出来的
...
- (void)swipeCellUpByByCellId:(int)cellId
{
    ...
    BOOL bEmpty = YES;// 上边没有格子
    for(int i = x-1;i>=0;i--) // 从这个格子的上边一个格子开始往上面判断
    {
        if([self.boxArr[i*N+y] intValue]> 0)// 有数字
        {
            // 数字相等 且 前面那个数字不是合并出来的
            if(curCellNumber == [self.boxArr[i*N+y] intValue] && ![self.mergeCellIdArr containsObject:@(i*N+y)])
            {
                // 相同 -> 合并
                int toCellId = i*N+y;
                [self mergeCellFrom:cellId To:toCellId];
...
- (void)swipeCellDownByCellId:(int)cellId
{
    ...
    BOOL bEmpty = YES;// 下边没有格子
    for(int i = x+1;i<N;i++) // 从这个格子的下边一个格子开始往上面判断
    {
        if([self.boxArr[i*N+y] intValue]> 0)// 有数字
        {
            // 数字相等 且 前面那个数字不是合并出来的
