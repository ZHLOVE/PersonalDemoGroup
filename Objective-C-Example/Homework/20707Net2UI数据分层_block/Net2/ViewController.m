...
//同步方式(封装)
- (IBAction)loginBtn4Pressed:(id)sender
{
    BOOL logined = [NetManager loginWithUserName:self.userNameTextfield.text andPasswd:self.pswdTextField.text];
    if (logined) {
        //提示登陆成功
        [SVProgressHUD showWithStatus:@"登陆成功"];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (IBAction)loginBtn5Pressed:(id)sender
{
    [SVProgressHUD showWithStatus:@"正在登陆"];
    [NetManager loginWithUserName:self.userNameTextfield.text
                        andPasswd:self.pswdTextField.text
                     successBlock:^{
                         [SVProgressHUD showWithStatus:@"登陆成功"];
                         dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                             [SVProgressHUD dismiss];
