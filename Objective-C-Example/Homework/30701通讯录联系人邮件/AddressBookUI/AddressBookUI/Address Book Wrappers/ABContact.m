...
{
    if (!ABPersonHasImageData(_record)) return nil;
    CFDataRef imageData = ABPersonCopyImageDataWithFormat(_record, kABPersonImageFormatThumbnail);
    if (!imageData) return nil;
...
//自己新增的赋值缩略图
- (void) setImageForThumbnail:(UIImage *)imageForThumbnail
{
    CFErrorRef errorRef = NULL;
    BOOL success;
    
    if (imageForThumbnail == nil) // remove
    {
        if (!ABPersonHasImageData(_record)) return; // no image to remove
        success = ABPersonRemoveImageData(_record, &errorRef);
        if (!success)
    ...
    NSData *data = UIImagePNGRepresentation(imageForThumbnail);
    success = ABPersonSetImageData(_record, (__bridge CFDataRef) data, &errorRef);
    if (!success)
    {
        NSError *error = (__bridge_transfer NSError *) errorRef;
