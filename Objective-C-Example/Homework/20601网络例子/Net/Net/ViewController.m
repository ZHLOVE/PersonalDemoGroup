...
- (void)viewDidLoad {
    ...
    NSURL *url = [NSURL URLWithString:@"http://127.0.0.1:8080/MJServer/login?username=123&pwd=&123&method=get&type=JSON"];
    NSLog(@"协议:%@",url.scheme);
    NSLog(@"主机:%@",url.host);
...
- (IBAction)btn2Pressed:(UIButton *)sender {
    ...
    //2请求对象
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
    //3发请求(异步请求，不会卡主线程，在后台请求数据，数据回来后，执行block里的处理方法)
    NSDate *d1 = [NSDate date];
