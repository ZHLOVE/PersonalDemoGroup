...
class VisitorView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        // 2.4设置按钮
        regisiterButton.xmg_AlignVertical(type: XMG_AlignType.BottomLeft, referView: messageLabel, size: CGSize(width: 100, height: 30), offset: CGPoint(x: 0, y: 20))
    ...
    //swift推荐我们自定义一个控件,要么纯代码，要么用xib或storyboard
    ...
    private lazy var loginButton:UIButton={
        ...
        btn.addTarget(self, action: nil, forControlEvents: UIControlEvents.TouchUpInside)
        return btn
    }()
    //注册按钮
    private lazy var regisiterButton: UIButton = {
        let btn = UIButton()
        btn.setTitleColor(UIColor.orangeColor(), forState: UIControlState.Normal)
