...
#ifdef DEBUG
static NSString *urlStr = @"http://192.168.1.105:5000/api"; //内测API
#else
...
{
    NSString *urlTemp = [urlStr stringByAppendingString:@"/employers"];
    NSString *urlTemp2 = [urlTemp stringByAppendingPathComponent:employer_id];
    NSString *urlString = [urlTemp2 stringByAppendingString:@"/periods"];
    NSString *tokenStr = [NSString stringWithFormat:@"token %@",token.access_token];
    [manager.requestSerializer setValue:tokenStr forHTTPHeaderField:@"Authorization"];
    [manager GET:urlString parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSArray *array = [responseObject objectForKey:@"periods"];
        NSMutableArray *per = [PeriodM mj_objectArrayWithKeyValuesArray:array];
        successBlock(per);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        NSString* errResponse = [[NSString alloc] initWithData:(NSData *)error.userInfo[AFNetworkingOperationFailingURLResponseDataErrorKey] encoding:NSUTF8StringEncoding];
        NSDictionary *dict = [errResponse mj_JSONObject];
