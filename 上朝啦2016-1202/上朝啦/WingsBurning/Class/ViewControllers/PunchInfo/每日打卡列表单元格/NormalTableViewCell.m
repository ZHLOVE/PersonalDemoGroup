...
- (void)setPunchGather:(PunchesGather *)punchGather{
    _dayLabel.text = punchGather.normal_days;
    _kgLabel.text = punchGather.absent_days;
    _cdLabel.text = punchGather.late_minutes;
    _ztLabel.text = punchGather.early_minutes;
}
...
- (void)loadUI{
    ...
    [self.kgUnitLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(10 * ratio);
        make.bottom.mas_equalTo(weakSelf.mas_bottom).offset(-23 * ratio);
        make.right.mas_equalTo(weakSelf.mas_right).offset(-35 * ratio);
    }];
