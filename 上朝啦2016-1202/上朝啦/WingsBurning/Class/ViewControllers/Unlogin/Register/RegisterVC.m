    ...
    if (isUserName & length) {
        if (isPhoneNumber) {
            ...
            [Networking zhuCe:emp successBlock:^(NSArray *arr) {
                ...
                //image转SHA1
                NSData *imgData = UIImageJPEGRepresentation(self.portraitImage.image, 0.7);
                ...
                [hud setMode:MBProgressHUDModeText];
                [hud setCenter:self.view.center];
                [hud setLabelText:NSLocalizedString(error, @"")];
...
- (UIButton *)captchaBtn{
    if (_captchaBtn == nil) {
        ...
        [_captchaBtn countDownButtonHandler:^(JKCountDownButton*sender, NSInteger tag) {
            BOOL length = ![_userNameTF.text isEqualToString:@""] ;
            BOOL isUserName = [CheckDataTool isChinese:_userNameTF.text];
            BOOL isPhoneNumber = [CheckDataTool checkForMobilePhoneNo:_phoneNumberTF.text];
            if (isUserName & length & isPhoneNumber) {
                sender.enabled = NO;
